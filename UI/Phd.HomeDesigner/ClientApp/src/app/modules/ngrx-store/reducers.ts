...
export const priceBreakdown = createSelector(
	...
	(salesAgreement, currentChangeOrder) => {
		...
		if (salesAgreement) {
			breakdown.salesPrice = salesAgreement.salePrice || 0;
			if (currentChangeOrder) {
				breakdown.changePrice = currentChangeOrder.changePrice || 0;
			}
			breakdown.totalPrice = breakdown.salesPrice + breakdown.changePrice;
		}

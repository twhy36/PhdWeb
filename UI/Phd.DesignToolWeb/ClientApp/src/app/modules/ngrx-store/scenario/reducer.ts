...
export const choicePriceRanges = createSelector(
	...
	(state) =>
	{
		...
		let getRelevantChoices = (ch: number, existing: number[] = []) =>
		{
			...
			var previousChoices = [
				..._.flatten(state.rules.choiceRules.filter(r => r.choiceId === ch).map(r => _.flatMap(r.rules, r1 => r1.choices))),
				..._.flatten(state.rules.pointRules.filter(r => currentChoice.treePointId === r.pointId).map(r => _.flatMap(r.rules, r1 => r1.choices && r1.choices.length ? r1.choices : _.flatMap(points.filter(pt => r1.points.some(p => p === pt.id)), p => p.choices[0].id)))),
			...
			if (previousChoices.length)
			{
				for (let perm of choicePermutations(previousChoices))
				{
					...
					for (let p of [...perm])
					{
						...
						{
							ch.quantity = 1;
							findPoint(staticTree, p => p.id === ch.treePointId).completed = true;
						}
					}

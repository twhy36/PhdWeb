...
export class LotService
{
	...
	getLot(lotId: number): Observable<LotExt>
	{
		...
		const expand = 'lotHandingAssocs($expand=handing($select=id,name)),planAssociations($select=id,isActive,planId,lotId),jobs($select=id,lotId,handing,planId),financialCommunity($select=id,name,number,city,state,zip,salesCommunityId,isPhasedPricingEnabled),salesPhase($expand=salesPhasePlanPriceAssocs($select=planId,price);$select=id)';

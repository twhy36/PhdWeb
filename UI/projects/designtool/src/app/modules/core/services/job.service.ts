...
export class JobService
{
	...
	getJobChangeOrderGroups(jobDto: IJob, salesAgreementId?: number): Observable<IJob>
	{
		...
		const jobChangeOrderPlanOptionAttributes = `jobChangeOrderPlanOptionAttributes($select=attributeName,attributeGroupLabel,manufacturer,sku,action)`;
		const jobChangeOrderPlanOptionLocations = `jobChangeOrderPlanOptionLocations($select=locationName,locationGroupLabel,quantity,action)`;
		...
		const expandPlanOptions = `jobChangeOrderPlanOptions($select=id,jobChangeOrderId,planOptionId,action,listPrice,optionSalesName,optionDescription,overrideNoteId,jobOptionTypeName;$expand=${jobChangeOrderPlanOptionAttributes},${jobChangeOrderPlanOptionLocations},${planOptionCommunity})`;

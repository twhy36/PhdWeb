...
import
{
	...
	SalesStatusEnum, ChangeOrderGroupSalesStatusHistory, SalesNotesChangeOrders, Note, isSalesChangeOrder, Constants
} from 'phd-common';
...
export function reducer(state: State = initialState, action: ChangeOrderActions): State
{
	switch (action.type)
	{
		...
		{
			const newChangeInput = _.cloneDeep(state.changeInput);
		...
		case CommonActionTypes.ESignEnvelopesLoaded:
			const changeOrder = new ChangeOrderGroup(state.currentChangeOrder);
		...
		case CommonActionTypes.JobLoaded:
		{
			let isPendingChangeOrder = action.changeOrder && action.changeOrder.salesStatusDescription === 'Pending'
				...
				&& (action.type === ChangeOrderActionTypes.CurrentChangeOrderLoaded
					...
					|| (action.salesAgreement && [Constants.AGREEMENT_STATUS_PENDING, Constants.AGREEMENT_STATUS_OUT_FOR_SIGNATURE, Constants.AGREEMENT_STATUS_SIGNED].indexOf(action.salesAgreement.status) === -1)
				);
		...
		case ChangeOrderActionTypes.SetIsChangeOrderComplete:
			return { ...state, isChangeOrderComplete: action.isChangeOrderComplete };
		...
		case ChangeOrderActionTypes.SetChangeOrderTrustName:
		{
			...
			const newChangeOrder = { ...state.currentChangeOrder };
		...
		{
			const newInput = _.cloneDeep(state.changeInput);
			const buyers = newInput.buyers as Array<Buyer>;

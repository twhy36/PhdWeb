					...
					if (result.isPhdLiteEnabled && (!result.action.tree || this.liteService.checkLiteAgreement(result.action.job, result.action.changeOrder)))
					{
						...
						return combineLatest([
							...
							switchMap(([options, optionsForCategories]) =>
							{
								...
								if (store.salesAgreement.status === SalesAgreementStatuses.Signed || store.salesAgreement.status === SalesAgreementStatuses.OutForSignature)
								{
								...
								else if (store.salesAgreement.status === SalesAgreementStatuses.Pending || !!store.job.id)
								{

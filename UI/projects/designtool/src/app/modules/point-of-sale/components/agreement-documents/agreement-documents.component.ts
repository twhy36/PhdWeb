...
import { UnsubscribeOnDestroy, ESignTypeEnum, Constants } from 'phd-common';
	...
	ngOnInit()
	{
		...
		).subscribe(([templates, selectedTemplates, salesAgreementStatus, isLockedIn, co]) =>
		{
			...
			this.contractTemplates = templates
				.filter(t => t.templateTypeId !== TemplateTypeEnum['Cancel Form'] || (salesAgreementStatus !== Constants.AGREEMENT_STATUS_PENDING && t.templateTypeId == TemplateTypeEnum['JIO Form']))

...
export class ManageTreeComponent extends ComponentCanNavAway implements OnInit, OnDestroy
{
	...
	async onNewTreeClick(from: string)
	{

		if (from === 'Catalog')
		{
			const confirmDefaultOption = 'Continue';
			if (await this.showConfirmModal('Creating a new draft from the catalog will clear all existing tree content', 'Attention', confirmDefaultOption))
			{
				this.createNewTree(from);
			}
		}
		else
		{
	...
	createNewTree(from: string)
	{
		...
		this._treeService.getNewTree(commId, planKey, from)
			.pipe(
				switchMap(tree =>
				{
